...
void Gui::handleInput(sf::Keyboard::Key name) {
    if (isClicked) {
        if (name == 8) {
            if (!size.empty()) {
                size.pop_back();
        ...
        if (passed) {
            ...
            for (auto& rec : temp) {
                ...
                rec.setPosition({800.f - determine * moveDistance / 2 + move,
                                 150.f});
...
void Gui::handlePlayerInput(sf::Mouse::Button key, bool pressed) {
    ...
    if (sorted && mousePosition.y >= 70.f && mousePosition.y <= 100.f) {
        if (mousePosition.x >= 25.f && mousePosition.x <= 200.f &&
        ...
        } else if (sorted && mousePosition.x >= 1480.f &&
                   mousePosition.x <= 1580.f) {
            sorted = false;
        ...
        } else if (mousePosition.x >= 700.f && mousePosition.x <= 800.f &&
                   pressed) {
            bubbleSortNow = true;
            ...
            quickSortNow = false;
            left = 0;
        } else if (mousePosition.x >= 1165.f && mousePosition.x <= 1230.f &&
                   pressed) {
            insertionSortText.setColor(sf::Color::White);
            bubbleSortText.setColor(sf::Color::White);
